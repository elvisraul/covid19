...
function loadHistorialtemperature(){
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT 
            ...
            CASE WHEN P.action='S' THEN CAST ( R.temperature AS varchar ) ELSE CAST ( R.temperature AS varchar ) END AS temperature
            FROM emergency.recurrence R 
            INNER JOIN emergency.permanence P ON R.id=P.id_recurrence
...
function putTemperature(){
    ...
    try {
        ...
        $stmtx = $pdo->prepare("SELECT R.id_visit,RR.id id_recurrence,RRR.temperature FROM emergency.recurrence R 
            INNER JOIN (SELECT RR.id_visit, MIN(RR.id) AS id FROM emergency.recurrence RR GROUP BY RR.id_visit ) RR ON R.id_visit=RR.id_visit
            INNER JOIN (SELECT RR.id,RR.id_visit, RR.temperature FROM emergency.recurrence RR) RRR ON RR.id=RRR.id
            WHERE R.id=?
            GROUP BY R.id_visit,RR.id,RRR.temperature");
        $stmtx->execute([$idtemperature]);
        $rowsx = $stmtx->fetchAll(PDO::FETCH_OBJ);


        $id_visit=$rowsx[0]->id_visit;
        $id_recurrence_temperature=$rowsx[0]->id_recurrence;
        $recurrence_temperature=$rowsx[0]->temperature;

        if($recurrence_temperature==null){
            $stmtt = $pdo->prepare("UPDATE emergency.recurrence SET temperature=? 
            WHERE id=? ");
            $stmtt->execute([$temperature,$id_recurrence_temperature]);

            $stmtr = $pdo->prepare("UPDATE emergency.recurrence SET id_reason=?
            WHERE id_visit=? ");
            $stmtr->execute([$idreason,$id_visit]);
    ...
    try {
        ...
        if($row){
            ...
            if(($row->worker)=='SI'){
                ...
                $id_reason=1;//1-TRANSPORTISTA - CISTERNA / TRAILER / PUBLICO
                $arr = array( 'worker'=>'no');
            }
            ...
            $ur = "http://localhost/apisubject/persona/".$dni;
